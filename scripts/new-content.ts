...
function main() {
  ...
  if (!collectionName || !(collectionName in COLLECTIONS)) {
    console.error(`Usage: npm run <collection>:new -- "<title>"`);
    console.error(`  npm run weeknotes:new -- "Jan 26th"`);
    console.error(`  npm run blog:new -- "my blog post title"`);
    console.error(`  npm run notes:new            (title-less, no input needed)`);
    if (collectionName && !(collectionName in COLLECTIONS)) {
  ...
  if (!titleless && !input) {
    console.error(
      `A title is required: npm run ${collectionName}:new -- "<title>"`,
    );
    process.exit(1);
  }
